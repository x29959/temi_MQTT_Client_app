...
//        sRobot.addOnBeWithMeStatusChangedListener(this);
    ...
    public void onConnect(View v) {
        ...
        if (mMqttClient != null && mMqttClient.isConnected() && hostUri.equals(mMqttClient.getServerURI())) {
            Toast.makeText(MainActivity.this, "已連接成功", Toast.LENGTH_SHORT).show();
    ...
    public void robotPublishStatus() throws JSONException {
        ...
        // asr result message
        payload.put("asr_result", collectedSpeech);
    ...
    private void parseMessage(String topic, JSONObject payload) throws JSONException {
        ...
        if (robotID.equals(sSerialNumber)) {
            switch (category) {
                ...
                //asr request
                case "asr":
                    sRobot.askQuestion(payload.getString("utterance"));
                    break;
                case "media":
    ...
    private String collectedSpeech = "";
    ...
    public void onAsrResult(@NotNull String s) {
        ...
        try {
            if (mMqttClient != null && mMqttClient.isConnected()) {
                MqttMessage message = new MqttMessage(collectedSpeech.getBytes());
                mMqttClient.publish("temi/" + sSerialNumber + "/status/result", message);
            }
